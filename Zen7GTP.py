...
def sm2str(s, size):
    if not s:
        return 'PASS'
    return (
        f'{"ABCDEFGHJKLMNOPQRST"["abcdefghijklmnopqrs".find(s[0])]}'
        f'{size - "abcdefghijklmnopqrs".find(s[1])}'
    )
        ...
        if topB and topB != topA:
            ...
            show(
                ...
                + '\n'.join(  # GTP Shell
                    [
                        (
                            f' {"!" if itemB[4].split()[0] == sgfmove else " "} '
                            f'{int2bw(c)} '
                            f'{itemB[4].split()[0]:<4}'
                            '-> '
                            f'{itemB[2]:>8} '
                ...
                if sm == 'PASS':
                    ...
                    if sm in mlist0:
                        ...
                        if mlist[m]:
                            sgfmod2 = (
                                f'\n(;{int2bw(c)}'
                                f'[{"abcdefghijklmnopqrstuvwxyz"[x]}{"abcdefghijklmnopqrstuvwxyz"[y]}]'
                                f'N[book {float(mlist[m][0]):.1f}]'
                                f'C[{float(mlist[m][0]):.1f}]'
                            )
                        else:
                            sgfmod2 = (
                                f'\n(;{int2bw(c)}'
                        ...
                        while True:
                            ...
                            if mlist1[m1]:
                                sgfmod2 += (
                                    f';{int2bw(c1)}'
                                    f'[{"abcdefghijklmnopqrstuvwxyz"[x]}{"abcdefghijklmnopqrstuvwxyz"[y]}]'
                                    f'N[book {float(mlist1[m1][0]):.1f}]'
                                    f'C[{float(mlist1[m1][0]):.1f}]'
                                )
                            else:
                                sgfmod2 += (
                                    f';{int2bw(c1)}'
        ...
        if sm == 'PASS':
            ...
            if top[0][4].split()[0] == 'pass':
                ...
                sgfmod += (
                    ...
                    + (
                        '[]'
                        f'N[{top[0][3] * 100 if c == 2 else (100 - top[0][3] * 100):.1f}]'
                        f'C[{top[0][3] * 100 if c == 2 else (100 - top[0][3] * 100):.1f}]'
                    )
                )
            else:
            ...
            if top[0][4].split()[0] == sm:
                ...
                sgfmod += (
                    ...
                    + ']'
                    + (
                        f'N[{top[0][3] * 100.0 if c == 2 else (100 - top[0][3] * 100.0):.1f}]'
                        f'C[{top[0][3] * 100.0 if c == 2 else (100 - top[0][3] * 100.0):.1f}]'
                    )
                )
            else:
